...
pub fn isError(err: anyerror) bool {
    inline for (@typeInfo(Error).error_set orelse unreachable) |set_error|
...
pub fn toError(err: anyerror, fallback: Error) Error {
    return if (isError(err)) @errorCast(err) else fallback;
}
